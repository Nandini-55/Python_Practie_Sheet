...
# #Given an array and an increment no. - generate a new array containing all the values of the original array wiht the increment value

# nums = list(map(int,input().split()))
# incre=int(input("Increment no. : "))
# result=[]
# for i in range(len(nums)):
#     result.append(nums[i]+incre)
#     print(result[i],end=" ")

# Given an aray filter out odd no. 
nums = list(map(int,input().split()))
odds=[]
for i in range(len(nums)):
    if(nums[i]%2!=0):
